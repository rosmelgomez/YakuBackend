...
void registrarLecturaDistancia(float distancia_cm);
void apagarBomba();
void apagarValvula();
...
const uint8_t LECTURAS_SENSOR_ERROR_MAX = 3;
const unsigned long INTERVALO_MEDICION_SENSOR_MS = 1000UL;
float ultimaDistanciaValida = -1;
uint8_t lecturasSensorFallidas = 0;
...
void registrarLecturaDistancia(float distancia_cm) {
  if (distancia_cm > 0) {
    ultimaDistanciaValida = distancia_cm;
    lecturasSensorFallidas = 0;
  } else {
    lecturasSensorFallidas++;
  }
}
  ...
  if (topicStr == configTopic) {
    ...
    if (funcionamientoActivo) {
      if (accion == "ON" || accion == "1" || accion == "HIGH") {
        ...
        registrarLecturaDistancia(d_actual);
...
void loop() {
  ...
  if (id_asignacion_proximidad > 0) {
    unsigned long intervaloMedicion = INTERVALO_MEDICION_SENSOR_MS;
  ...
  if (id_asignacion_proximidad <= 0) {
    ...
    if (d < 0 && lecturasSensorFallidas >= LECTURAS_SENSOR_ERROR_MAX) {
      if (bombaActiva) {
      ...
      Serial.printf("Lectura de sensor fallida %u/%u; se conserva estado seguro previo\n", lecturasSensorFallidas, LECTURAS_SENSOR_ERROR_MAX);
  ...
  bool cambioBomba = bombaActiva != ultimoEstadoBomba;
  bool cambioValvula = valvulaAbierta != ultimoEstadoValvula;
  bool eventoRiego = cambioBomba || limpiarCicloDespuesPublicar || (publicarEstadoInmediato && bombaSolicitada);
  bool eventoTanque = cambioValvula;
  if (eventoRiego || eventoTanque) {
