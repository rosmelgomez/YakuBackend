...
void mostrarEstado() {
  ...
  lcd.setCursor(0, 1);
  // Dos pantallas alternadas: evitar desbordar los 16 caracteres con volumen alto.
  char linea[17];
  if ((millis() / 3000) % 2 == 0)
    snprintf(linea, sizeof(linea), "Q:%.3f L/s", caudal);
  else
    snprintf(linea, sizeof(linea), "V:%.3f L", litrosRiego);
  lcd.print(linea);
}
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("\n=== Yaku ESP32 Flujo v1.0.7 – Caudal L/s ===");
