...
// El cronometro del ciclo vive en el equipo: cierra exactamente en duracion_seg
// aunque no haya conexion. El backend extrapola con la misma duracion, asi que
// la app y el equipo terminan juntos sin depender de que llegue un OFF.
...
int32_t idRiego = 0; // Sesion 'riego' del backend que origino el ciclo (0 = desconocida).
// litrosEvento: volumen del riego en curso; sigue sumando sin conexion y vuelve
// a 0 solo cuando el backend recibio el cierre. litrosTotal: suma historica de
// eventos del agricultor, persistida en NVS.
float litrosEvento = 0, litrosTotal = 0, caudal = 0; // Caudal siempre en L/s.
...
void mostrarEstado() {
  ...
  if (activo && (ahora - ultimoMonitorMs >= 5000 || !regando)) {
    ...
    Serial.printf("  [Flujometro] Pulsos=%lu | Caudal=%.4f L/s | Ciclo=%.3f L\n",
        (unsigned long)pulsosMedidos, caudal, litrosEvento);
    Serial.printf("✅ Riego=%s | Consumido=%.3f L | Acumulado=%.3f L\n",
        regando ? "ON" : "OFF", litrosEvento,
        litrosTotal + (regando ? litrosEvento : 0.0f));
  }
  if (!lcdDisponible) return;
  lcd.setCursor(0, 0);
  if (regando) {
    // "ON Ev:" + 9 de volumen del evento + "L" = 16 columnas.
    char cabecera[24];
    snprintf(cabecera, sizeof(cabecera), "ON Ev:%9.2fL", litrosEvento);
    lcd.print(strlen(cabecera) == 16 ? cabecera : "ON Ev:#########L");
  } else {
    lcd.print("Riego directo OFF");
  }
  lcd.setCursor(0, 1);
  // 7 columnas de volumen total + espacio + 8 de caudal, siempre juntos.
  // Sobrescribir las 16 columnas evita parpadeo y restos de cifras anteriores.
  char volumen[24], flujo[24];
  float totalAcumulado = litrosTotal + (regando ? litrosEvento : 0.0f);
...
void cerrarRiego(const char* motivo) {
  ...
  if (regando) {
    ...
    litrosEvento = calcularLitros(pulsosMedidos, factorRiego);
    litrosTotal += litrosEvento;
  ...
  motivoCierre = motivo;
  pendiente = true;
  // La habilitacion (activo) solo la cambia el usuario via /config: cerrar la
  // valvula no deshabilita el equipo. Antes se ponia activo=false en cada
  // cierre y el equipo dejaba de enviar su estado de reposo (se veia offline
  // y el backend no podia detectar un ON perdido).
  mostrarEstado();
}
...
bool publicarEstado() {
  ...
  if (idRiego > 0) doc["id_riego"] = idRiego;
  ...
  // Nombres legados: litros_riego = evento, litros_acumulados = total.
  doc["litros_riego"] = litrosEvento;
  doc["litros_acumulados"] = litrosTotal + (regando ? litrosEvento : 0.0f);
  ...
  if (ok) {
    ...
    if (cierrePendiente) {
      ...
      // pasara el cooldown. litrosEvento vuelve a tomar valor real solo al
      // iniciar el proximo ciclo (ver linea "litrosEvento = caudal = 0;" en
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == configTopic) {
    ...
    if (doc.containsKey("litros_acumulados")) {
      // El total local (NVS) manda. El del backend solo lo recupera si es mayor
      // (p.ej. NVS borrada) y nunca durante un evento o con su cierre pendiente:
      // ese valor ya incluye litros del evento en curso y, al cerrarlo, se
      // sumarian dos veces. Un mensaje retenido viejo (menor) tampoco lo baja.
      float remoto = doc["litros_acumulados"].as<float>();
      if (!regando && !cierrePendiente && isfinite(remoto) && remoto > litrosTotal) {
        litrosTotal = remoto;
        prefs.begin("yaku_directo", false);
        prefs.putFloat("litros_tot", litrosTotal);
        prefs.end();
  ...
  // El backend solo envia ON a asignaciones activas; si la configuracion
  // retenida aun no llego tras un reinicio, la orden misma habilita el equipo.
  activo = true;
  ...
  // Sin margen: el equipo cierra solo al cumplir la duracion pedida, con o sin
  // conexion, en el mismo instante en que la app muestra el fin del ciclo.
  duracionMs = (uint32_t)min(segundos, (int)MAX_RIEGO_SEG) * 1000UL;
  idRiego = doc["id_riego"] | 0;
  inicioMs = ultimoReporteMs = millis();
  ejecutadoMs = 0;
  pulsosCiclo = pulsosAnterior = leerPulsos();
  pulsosMedidos = 0;
  factorRiego = pulsosPorLitro;
  litrosEvento = caudal = 0;
  motivoCierre = "";
  regando = true;
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("\n=== Yaku ESP32 Flujo v1.0.13 – Estado de reposo continuo tras cada riego ===");
